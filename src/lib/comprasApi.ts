...
async function buscarLicitacoesPorModalidade(
 ...
 console.log(`ℹ️  Iniciando busca para modalidade código: ${modalidadeCode}`);
...
export async function buscarLicitacoesPNCP(
 ...
 try {
  ...
  if (cachedData) {
   ...
   console.log(`🚀 Disparando buscas em paralelo para ${modalidadesCodigos.length} modalidades.`);
   const promises = modalidadesCodigos.map(modalidadeCode =>
    buscarLicitacoesPorModalidade(modalidadeCode, baseParams)
   );
  ...
  console.log(`✅ Total de ${allLicitacoes.length} licitações brutas (do cache ou da API).`);
